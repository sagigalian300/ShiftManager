...
const VisualizeWholeWeek = ({ week_id, onClose }) => {
  ...
  useEffect(() => {
    if (week_id) {
      console.log("Fetching week data for week_id:", week_id);
      getWeekDataForExcelDocument(week_id).then((data) => {
        const raw = data?.weekData;
        // Normalize: allow backend to return an array or the usual object
        if (Array.isArray(raw)) {
          setWeekData({
            days: raw,
            start_date: data?.start_date ?? raw.start_date ?? "",
          });
        } else if (raw && Array.isArray(raw.days)) {
          setWeekData(raw);
        } else {
          setWeekData({ days: [], start_date: data?.start_date ?? "" });
        }
      });
    }
  }, [week_id]);
  ...
  // When rendering, this now passes even if days is an empty array
  ...
  return (
    ...
    <div
      id="print-section"
      className="fixed inset-0 z-50 flex flex-col h-screen w-screen bg-gray-50 text-gray-800 overflow-hidden font-sans"
      dir="rtl"
    >
      {/* --- Header --- */}
                      ...
                      day.shifts.map((shift, sIndex) => {
                        ...
                        return (
                          ...
                          >
                            {/* כותרת משמרת */}
                            <div
                              className={`flex justify-between items-center px-1.5 py-1 border-b ${styles.header} ${styles.border}`}
                            >
                            ...
                            <div className="p-0.5 space-y-0.5">
                              {assignments.length === 0 ? (
                                <div className="text-[9px] text-center opacity-40 py-0.5">
                                  {t("emptyAssignments")}
                                </div>
                              ) : (
                                assignments.map((assign, aIndex) => (
                                  ...
                                  >
                                    <div className="flex items-center gap-1 overflow-hidden">
                                      <Briefcase
                                        size={9}
                                        className={`${styles.roleIcon} shrink-0`}
                                      />
                                      <span
                                        className="text-[10px] text-gray-600 truncate max-w-[45px]"
                                        title={assign?.roles?.name}
                                      >
                                        {assign?.roles?.name}
                                      </span>
                                    </div>
                                    <div className="flex flex-col items-center shrink-0">
                                      <span
                                        className="text-[10px] font-bold text-gray-800 truncate max-w-[60px]"
                                        title={`${assign?.workers?.first_name} ${assign?.workers?.last_name}`}
                                      >
